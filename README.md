...
Pylint-Ignore doesn't get rid of this noise of course, but it does put in a dedicated place, rather than Spam in your terminal. It gives the developer control, who can just ignore and commit if they feel that this is justified by their current priorities.
...
Even if you've setup Pylint perfectly and done cleanup of your codebase, there might be reason for you to continue to use Pylint-Ignore in your development workflow.
...
With Pylint-Ignore you can keep your production train going and use the `pylint-ignore.md` as a measure of progress.


### Problem 5: False positives

While you can and should deal with most false positives using `disable` comments, there are some cases where you'd rather not do that and [some cases where that isn't even possible](https://github.com/PyCQA/pylint/issues/214). For such edge cases, you can just permanently leave an entry in the `pylint-ignore.md` and still benefit from an otherwise useful message.
...
The above code will "work", depending on how you call it, it won't even throw an error and yet it is almost certainly not correct. Were the function longer than 5 lines, the bug would perhaps be less obvious. Just recently I spent 2 hours tracking down such a bug that had made it into production. In any case, Pylint will report the following message for such code:
...
To the extent that the excessive noise of Pylint has scared you away from using it, I hope you will find `pylint-ignore` helpful to at least get started. There are other approaches that you may prefer.
...
The `pylint-ignore` instead works by monkey-patching the Pylint module to intercept messages as they are generated and capture/ignore them. The Pylint command remains available to use as-is and your project files are not modified in any way.
